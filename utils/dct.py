...
class DCT(nn.Module):
    def __init__(self, k: int = 1):
        ...
        self.cbcr_lpf = torch.cat([self.y_lpf, self.y_lpf], dim=0)
        self.cbcr_lpf.requires_grad = False  # 勾配は不要
    ...
    def forward(self, images: torch.Tensor, k: int = 1) -> torch.Tensor:
        ...
        adaptted_cbcr = coefs_cbcr * self.cbcr_lpf
        adaptted_y = coefs_y * self.y_lpf
        rgb_shifted = self.decoder((adaptted_y, adaptted_cbcr))
        rgb = rgb_shifted * 2 - 1
        return rgb
